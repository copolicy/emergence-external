...
export default function Signal({ controlsTarget = null }: SignalProps = {}) {
  ...
  const [params, setParams] = useState<SignalParams>(DEFAULT_SIGNAL);
  ...
  const lines = useMemo(() => computeSignal(w, h, params), [params, w, h]);
  const exportLines = useMemo(
    () => computeSignal(exportDims.w, exportDims.h, exportParams),
  ...
  }, [lines, ink, background, growth, w, h, fade, params.seed]);
  ...
  const getExportRender = useCallback(
    ...
    [exportDims, exportLines, ink, background, growth, fade, params.seed],
  );
    ...
    return (
      ...
      >
        ...
        <input
          ...
          value={value}
          onChange={(e) =>
            updateParam(key, +e.target.value as SignalParams[typeof key])
          }
        />
      </label>
    );
